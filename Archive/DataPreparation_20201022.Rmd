...
AnimalID_TimePeriod_Last<- AnimalID_TimePeriod_AfterFirst  %>%
  #GROUP BY 
  group_by(AnimalID) %>% 
...
LHAMT_Urine <- AnimalID_AvailablePeriod %>% 
...
#Count of samples for each period of interest 
...
LHAMT_Urine_PeriodCount_All<- PeriodList %>% 
  left_join(.,LHAMT_AnimalInfo_All %>% 
              select(AnimalID,Sex,Queried_BreedingStatus)) %>% 
  left_join(LHAMT_Urine_PeriodCount) %>% 
  replace_na(list(SampleCount = 0))
View(LHAMT_Urine_PeriodCount_All)
...
#Female
LHAMT_Female_Urine_Figure<- ggplot(LHAMT_Urine_PeriodCount_All %>% 
                              filter(Sex == "F"), aes(x=EndPeriod,y=SampleCount)) +
  geom_boxplot() +
  geom_line(aes(group = AnimalID), alpha = 0.2) +
  facet_grid(rows = vars(Queried_BreedingStatus))+
  theme_bw() +
...
my_pres<-
  ...
  print(.,target="LHAMT_Urine.pptx")


#Male
LHAMT_Male_Urine_Figure<- ggplot(LHAMT_Urine_PeriodCount_All %>% 
...
LHAMT_Urine_Selection <- LHAMT_Urine %>% 
...
LHAMT_Plasma <- AnimalID_AvailablePeriod %>% 
  left_join(.,PlasmaUrine %>% 
...
#Count of samples for each period of interest 
#This will not show the period during which the animal may have been available but no sample were collected
LHAMT_Plasma_PeriodCount <- LHAMT_Plasma %>% 
  #GROUP BY ANIMALID, SEX, BREEDINGSTATUS,PERIOD, PERIOD DATE
group_by(AnimalID,Sex,Queried_BreedingStatus,EndPeriod,EndPeriodDate,EndPeriodType) %>% 
  summarize(SampleCount = n_distinct(SampleID)) %>% 
  ungroup() %>% 
...
#Add period for which there may be zeros
LHAMT_Plasma_PeriodCount_All<- PeriodList %>% 
  left_join(.,LHAMT_AnimalInfo_All %>% 
              select(AnimalID,Sex,Queried_BreedingStatus)) %>% 
  left_join(LHAMT_Plasma_PeriodCount) %>% 
  replace_na(list(SampleCount = 0))
View(LHAMT_Plasma_PeriodCount_All)


##################################################Graphical representation 

#Female
LHAMT_Female_Plasma_Figure<- ggplot(LHAMT_Plasma_PeriodCount_All %>% 
                              filter(Sex == "F"), aes(x=EndPeriod,y=SampleCount)) +
  geom_boxplot() +
  geom_line(aes(group = AnimalID), alpha = 0.2) +
  facet_grid(rows = vars(Queried_BreedingStatus))+
...
my_pres<-
  ...
  print(.,target="LHAMT_Plasma.pptx")


#Male
LHAMT_Male_Plasma_Figure<- ggplot(LHAMT_Plasma_PeriodCount_All %>% 
                              filter(Sex == "M"), aes(x=EndPeriod,y=SampleCount)) +
  geom_boxplot() +
  geom_line(aes(group = AnimalID), alpha = 0.2) +
  facet_grid(rows = vars(Queried_BreedingStatus))+
...
LHAMT_Plasma_Selection <- LHAMT_Plasma %>% 
  select(AnimalID,CollectionDate,SampleID) %>% 
  left_join(.,Samples_Exported %>% 
              filter(SampleType == "Plasma") %>% 
              select(SampleNumber,Date_Exported,Purpose,DestinationLab), by=c("SampleID" = "SampleNumber"))
